    ...
    var BOXCoin = {
        ...
        checkout: {
            settings: function (checkout) {
                checkout = _(checkout);
                let checkout_id = checkout.data('boxcoin');
            ...
            init: function (settings, area, onSuccess = false) {
                ...
                _.ajax(BXC_URL + 'init.php', { checkout: settings, language: language, cloud: cloud }, (response) => {
                    ...
                    if (active_transaction) {
                        active_checkout_id = area.data('boxcoin');
            ...
            cancelTransaction: function (delete_db_transaction = false) {
                if (delete_db_transaction && !BOXCoin.getURL('demo') && !this.storageTransaction(active_checkout_id).prevent_cancel) {
                    ajax('cancel-transaction', { transaction: this.storageTransaction(active_checkout_id).encrypted });
                }
                clearInterval(intervals[active_checkout_id]);
                clearInterval(intervals['check-' + active_checkout_id]);
            ...
            storageTransaction: function (checkout_id, transaction = false) {
                ...
                if (transaction) {
                    if (transaction == 'delete') {
                        ...
                        if (exists) {
                            transaction = Object.assign(transaction, transactions[checkout_id]);
        ...
        network: function (cryptocurrency_code, label = true, exclude_optional = false) {
            ...
            if (exclude_optional && ['btc', 'eth', 'ltc', 'xrp', 'algo', 'bch', 'doge', 'bnb'].includes(cryptocurrency_code)) {
                return '';
            }
            for (var key in networks) {
                if (networks[key].includes(cryptocurrency_code)) {
                    if (code) return key.toLowerCase();
                    let text = key.replace('TRX', 'TRON') + ' ' + bxc_('network');
